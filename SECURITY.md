...
- The app makes no third-party requests or location prompts merely on page load. Scanning, location, and tests require a user action or an explicitly invoked agent tool. Credentials and referrers are omitted from test requests.
- When embedded, the app refuses network operations and does not load history or register agent tools. This is a fallback, not a replacement for a `frame-ancestors` response header.
