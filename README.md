...
A local, dependency-free web app with sustained measurements, location-aware server selection, live graphs, a rocket animation, connection ratings, and test history.
...
- IP and ISP discovery uses Cloudflare and the previously approved RIPEstat lookup. Your public IP is sent to RIPEstat to identify the registered network holder. A VPN or upstream network may be shown instead of a retail ISP brand. Missing metadata does not prevent a test.
...
- `dist/rocket.png`: generated transparent rocket artwork.
...
Change `MEASURE_MS`, request sizes, or rating thresholds to experiment. Update data-use notices when changing measurement duration. Add city centers to `servers.json` for new catalog locations. Optional feature-detected WebMCP uses the same Start Test flow.
